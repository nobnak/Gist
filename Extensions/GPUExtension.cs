...
namespace nobnak.Gist.Extensions.GPUExt {
	...
	public static class GPUExtension {
		...
		public static Vector3Int CeilSize(this ComputeShader cs, int kernel, Vector3Int counts) {
			uint gsx, gsy, gsz;
			cs.GetKernelThreadGroupSizes(kernel, out gsx, out gsy, out gsz);
			return new Vector3Int(
				(int)gsx * DispatchSize(counts.x, (int)gsx),
				(int)gsy * DispatchSize(counts.y, (int)gsy),
				(int)gsz * DispatchSize(counts.z, (int)gsz));
		}
	}
}
